...
Figure \@ref(fig:effects-pcc-d) depicts the relationship between the effect size of time 1 minus time 2 and the corresponding PCC values. These metrics appear to represent different concepts where effect size measures the magnitude of the difference between two data points while PCC disregards magnitude and represents the proportion of the sample following the given ordinal pattern across all three time points. Given these differences, it is interesting how well converge together. 

We can see that as the simulated effect size increases across plot panels, estimates for the effect size are less likely to fall in the negative range and more likely to grow in size. 
